import { BookOpen, CircleAlert, ClipboardCheck, FileBox, FlaskConical, GitBranch, GraduationCap, ListChecks, PackageCheck, ShieldCheck, UserRoundCog, X } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 bg-slate-950/60 p-3 backdrop-blur-sm sm:p-4" role="dialog" aria-modal="true" aria-label="Lab dashboard">
      <div className="mx-auto flex h-full max-w-7xl flex-col overflow-hidden rounded-[1.5rem] border border-slate-200 bg-[#f6f7fb] shadow-2xl sm:rounded-[2rem]">
        ...
        <main className="min-h-0 flex-1 overflow-y-auto p-5 sm:p-6 lg:p-8">
          ...
          <section className="mt-8 grid gap-4 lg:grid-cols-[1.1fr_.9fr]"><div className="rounded-2xl border border-amber-200 bg-amber-50 p-6"><div className="flex items-center gap-2 text-amber-800"><CircleAlert size={17} /><span className="text-[10px] font-black tracking-[.16em]">OPEN QUESTIONS</span></div><div className="mt-4 space-y-3">{openQuestions.map((question, index) => <div key={question} className="flex gap-3 rounded-xl border border-amber-200/70 bg-white/60 p-3"><span className="text-xs font-black text-amber-700">0{index + 1}</span><p className="text-sm font-semibold leading-6 text-amber-950">{question}</p></div>)}</div></div><div className="rounded-2xl border border-slate-200 bg-slate-950 p-6 text-white"><div className="flex items-center gap-2 text-indigo-300"><ClipboardCheck size={16} /><span className="text-[10px] font-black tracking-[.18em]">CURRENT CONTROL POINT</span></div><h3 className="mt-3 text-2xl font-black">Nothing ships just because the concept is attractive.</h3><p className="mt-3 text-sm leading-7 text-slate-300">Program approval, formulation governance, evidence, safety, regulatory, quality, personalization, pack-generation, fulfillment and education controls remain separate from the interface.</p><div className="mt-5 grid grid-cols-2 gap-2 text-xs font-bold"><div className="rounded-xl bg-white/10 p-3">FORMULATION<br /><span className="font-normal text-slate-400">{formulation.name}</span></div><div className="rounded-xl bg-white/10 p-3">OPERATIONS<br /><span className="font-normal text-slate-400">quality {quality.approved ? "approved" : "blocked"} · personalization {personalization.approved ? "approved" : "blocked"} · packs {packGeneration.approved ? "approved" : "blocked"} · fulfillment {fulfillment.ready ? "ready" : "blocked"} · education {education.approved ? "approved" : "blocked"}</span></div></div></div></section>
        </main>
      </div>
    </div>
  );
