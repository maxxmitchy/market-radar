import { Building2, CircleAlert, ClipboardCheck, FileBox, FlaskConical, GitBranch, GraduationCap, Hammer, ListChecks, MessageSquare, PackageCheck, PlayCircle, ShieldCheck, UserRoundCog, X } from "lucide-react";
...
import { feedbackSummary } from "../intelligence/feedback-engine.js";
...
import { pilotSummary } from "../intelligence/pilot-engine.js";
import { productSpecificationSummary } from "../intelligence/product-specification-engine.js";
...
const workstreams = [
  ...
  { title: "Feedback loop", state: "Prototype", detail: "Experience signals can be structured, reviewed and turned into governed product learning.", icon: MessageSquare },
  { title: "Pilot", state: "Prototype", detail: "The smallest controlled bridge from approved architecture to a physical test is defined without claiming readiness.", icon: PlayCircle },
  { title: "Product specification", state: "Prototype", detail: "The architecture is being translated into buildable product, software, physical and operating artifacts.", icon: Hammer },
];
const openQuestions = [
...
export function LabDashboard({ onClose }: { onClose: () => void }) {
  ...
  const specification = productSpecificationSummary();
  const total = summary.defined + summary.prototype + summary["needs-validation"] + summary.unresolved;
  return <div className="fixed inset-0 z-50 bg-slate-950/60 p-3 backdrop-blur-sm sm:p-4" role="dialog" aria-modal="true" aria-label="Lab dashboard">
    <div className="mx-auto flex h-full max-w-7xl flex-col overflow-hidden rounded-[1.5rem] border border-slate-200 bg-[#f6f7fb] shadow-2xl sm:rounded-[2rem]">
      ...
      <main className="min-h-0 flex-1 overflow-y-auto p-5 sm:p-6 lg:p-8">
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-[1.1fr_.9fr]"><div className="rounded-2xl border border-amber-200 bg-amber-50 p-6"><div className="flex items-center gap-2 text-amber-800"><CircleAlert size={17} /><span className="text-[10px] font-black tracking-[.16em]">OPEN QUESTIONS</span></div><div className="mt-4 space-y-3">{openQuestions.map((question, index) => <div key={question} className="flex gap-3 rounded-xl border border-amber-200/70 bg-white/60 p-3"><span className="text-xs font-black text-amber-700">0{index + 1}</span><p className="text-sm font-semibold text-amber-950 leading-6">{question}</p></div>)}</div></div><div className="rounded-2xl border border-slate-200 bg-slate-950 p-6 text-white"><div className="flex items-center gap-2 text-indigo-300"><ClipboardCheck size={16} /><span className="text-[10px] font-black tracking-[.18em]">CURRENT CONTROL POINT</span></div><h3 className="mt-3 text-2xl font-black">Nothing ships just because the concept is attractive.</h3><p className="mt-3 text-sm leading-7 text-slate-300">Program approval, formulation governance, evidence, safety, regulatory, quality, personalization, pack-generation, fulfillment, education, campus, feedback, pilot and product-specification controls remain separate from the interface.</p><div className="mt-5 grid grid-cols-2 gap-2 text-xs font-bold"><div className="rounded-xl bg-white/10 p-3">FORMULATION<br /><span className="font-normal text-slate-400">{formulation.name}</span></div><div className="rounded-xl bg-white/10 p-3">BUILD STATE<br /><span className="font-normal text-slate-400">quality {quality.approved ? "approved" : "blocked"} · pilot {pilot.ready ? "ready" : "blocked"} · specification {specification.approved ? "approved" : "in design"}</span></div></div></div></section>
      </main>
    </div>
  </div>;
}
